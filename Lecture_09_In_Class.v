...
Proof.
  ...
  contradiction.       (* Directly reach a contradiction because Void w := False, meaning no word can ever be in Void *)
Qed.
...
Proof.
  induction n; intros. {             (* Induction on n, breaking the proof into base case (n=0) and inductive step *)
    ...
    unfold In in *.                  (* Unfold the membership relation In to simplify the context *)
    ...
    subst;      (* Substitute any resulting equalities in the proof. For example, if the inversion step reveals that w = [], 
        this step will replace occurrences of w with []. *)
  ...
  subst.                             (* Substitute the equality w1 = [c] into the proof context *)
