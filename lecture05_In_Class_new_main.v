...
Check cons nat 1 (nil nat).  (* [1] *)
(* Output: cons nat 1 (nil nat) : list nat *)


Check cons nat 1 (cons nat 2 (nil nat)). (* = [1; 2] *)
...
Compute app (cons nat 1 (cons nat 2 (nil nat))) (cons nat 3 (cons nat 4 (nil nat))).
...
Inductive list2 (X : Type) : Type :=
  ...
  | cons2 : X -> list2 X -> list2 X. (* `cons2` is the constructor that takes an element of type X 
                                         and a list2 of type X, and produces a new list with
...
(* tells Coq that the parameter 
(usually a type parameter) for nil2 should be treated as implicit.
This means that when you use nil2 in your code, you don't have to
explicitly provide the type argument—the type will be automatically
inferred by Coq based on the context. *)
...
Compute app2 (cons2 1 (cons2 2 nil2)) (cons2 3 (cons2 4 nil2)).
...
Theorem eq_trans_v2 : forall (T : Type) (x y z : T),
  ...
  x = z.                           (* Goal: Prove `x = z` *)
Proof.
  intros.           (* Introduce the type `T` and variables `x`, `y`, and `z` 
                                       as well as the equality hypotheses `eq1` and `eq2`. *)
  rewrite -> H.                   (* Use `eq1` to rewrite `x` in terms of `y`. Now the goal becomes `y = z`. *)
  rewrite -> H0.                   (* Use `eq2` to rewrite `y` in terms of `z`. Now the goal becomes `z = z`. *)
...
injectivity of a function means that the function maps distinct 
elements of its domain to distinct elements of its codomain.
...
Proof.
  intros n eq1.                 (* Introduce n and eq1 : beq_nat 0 n = true. *)
  destruct n as [| n'].         (* Split into cases n = 0 or n = S n'. *)

  - reflexivity.                (* Case n = 0: goal is 0 = 0. *)

  - simpl in eq1.               (* beq_nat 0 (S n') simplifies to false, so eq1 becomes false = true. *)
    inversion eq1.              (* Impossible equality, so this case is eliminated. *)
Qed.
...
(**************************************************************************************)
(* Equality: transitivity via a provided transitivity function                           *)
(**************************************************************************************)

(* If a function can turn (x = y) and (y = z) into (x = z), then supplying those equalities
   yields x = z. This version uses rewrite and produces subgoals. *)
...
(* Same statement, proved by directly applying the transitivity function. *)
Theorem eq_trans_3_apply : forall (T : Type) (x y z : T),
  (x = y -> y = z -> x = z) ->
  x = y ->
  y = z ->
  x = z.
...
(**************************************************************************************)
(* Forward reasoning: applying an implication to a hypothesis                             *)
(**************************************************************************************)

(* If (beq_nat n 5 = true) implies (beq_nat (S (S n)) 7 = true), and true = beq_nat n 5,
   then true = beq_nat (S (S n)) 7. *)
...
Proof.
  intros n eq H.
  symmetry in H.          (* H : beq_nat n 5 = true *)
  apply eq in H.          (* H : beq_nat (S (S n)) 7 = true *)
  symmetry.               (* goal becomes beq_nat (S (S n)) 7 = true *)
  apply H.
Qed.


(**************************************************************************************)
(* Disjoint constructors: S n cannot equal 0                                              *)
(**************************************************************************************)

(* For all n, S n <> 0 (i.e., S n = 0 leads to False). *)
Theorem S_neq_0 : forall n : nat, S n <> 0.
Proof.
  intros n H.
  inversion H.
Qed.
...
(**************************************************************************************)
(* Targeted rewriting: rewrite changes the goal, apply closes it                          *)
(**************************************************************************************)

(* If x = y and y = z, then x = z. *)
Theorem target_hypothesis_demo : forall (x y z : nat),
  x = y ->
  y = z ->
  x = z.
Proof.
...
(**************************************************************************************)
(* Integers: keep Z scope local                                                           *)
(**************************************************************************************)

(* Local Open Scope Z_scope: makes numeric notations like 0, 1, +, * 
be interpreted as integers (Z) in the current region. *)

(* Close Scope Z_scope: ends that interpretation so later code
 uses the default scopes again (often nat). *)


Require Import ZArith.
Local Open Scope Z_scope.

(* Adding 0 on the right does not change an integer. *)
Theorem add_zero : forall n : Z, n + 0 = n.
Proof.
  intros n.
  apply Z.add_0_r.
Qed.
...
(************************************************************)
(* apply vs assumption: concrete examples with nat           *)
(************************************************************)

(*
Rule of thumb:

1) Use [assumption] when the goal already appears in the context
   (i.e., you already have a hypothesis whose type matches the goal).

2) Use [apply] when:
   - you want to use an implication (A -> B) to turn goal B into subgoal A, or
   - you want Coq to instantiate a universally quantified fact (forall x, ...), or
   - you want to use a lemma whose conclusion matches the goal after some unification.

Important note:
- If you have H : Goal, then BOTH of these finish the proof:
    assumption
    apply H
  In that special case, [apply] is not creating subgoals; it is just using H directly.
*)

(************************************************************)
(* 1) assumption closes the goal when it is exactly in context *)
(************************************************************)

Lemma assumption_closes_goal_nat :
  forall (x y : nat), x = y -> x = y.
Proof.
  intros x y H.              (* H : x = y, goal: x = y *)
  assumption.                (* finds H and closes *)
Qed.

(************************************************************)
(* 1b) apply can also close, because H already proves the goal *)
(************************************************************)

Lemma apply_closes_goal_nat :
  forall (x y : nat), x = y -> x = y.
Proof.
  intros x y H.              (* H : x = y, goal: x = y *)
  apply H.                   (* closes immediately because H matches the goal *)
Qed.

(************************************************************)
(* 2) apply changes the goal (symmetry turns y=x into x=y)   *)
(************************************************************)

Lemma apply_changes_goal_nat :
  forall (x y : nat), x = y -> y = x.
Proof.
  intros x y H.              (* H : x = y, goal: y = x *)
  apply eq_sym.              (* goal becomes: x = y *)
  assumption.                (* closes using H *)
Qed.

(************************************************************)
(* 3) apply uses an implication to create a new subgoal      *)
(************************************************************)

Lemma apply_creates_subgoal_nat :
  forall (x : nat),
    (x = 0 -> x + 1 = 1) ->
    x = 0 ->
    x + 1 = 1.
Proof.
  intros x Himp Hx0.         (* goal: x + 1 = 1 *)
  apply Himp.                (* goal becomes: x = 0 *)
  assumption.                (* closes using Hx0 *)
Qed.

(************************************************************)
(* 3b) apply can instantiate a forall; assumption cannot     *)
(************************************************************)

Lemma apply_instantiates_forall_nat :
  (forall n : nat, n = n) -> 3 = 3.
Proof.
  intro Hall.                (* Hall : forall n, n = n, goal: 3 = 3 *)

  (* If you try:
     Fail assumption.
     It fails because there is no hypothesis exactly "3 = 3". *)
  

  apply Hall.                (* instantiates n := 3, goal solved *)
Qed.

(************************************************************)
(* 4) apply can split the goal into two goals (transitivity) *)
(************************************************************)

Lemma apply_splits_into_two_goals_nat :
  forall (x y z : nat), x = y -> y = z -> x = z.
Proof.
  intros x y z Hxy Hyz.
  apply eq_trans with (y := y).   (* creates subgoals: x = y and y = z *)
  - assumption.
  - assumption.
Qed.
